...
class EEDBOperationPred:
    ...
    def __init__(self):
        ...
        self.log_path = "EElogging/prediction/EEDBOperationPred.txt"
        self.good_file_dir = "DIV/PredictionData/GoodRaw/"
        self.bad_file_dir = "DIV/PredictionData/BadRaw/"
        self.table_name = "good_pred_data"

    def ee_db_connection(self):
        """
        :Method Name: ee_db_connection
        :Description: This method connects to the keyspace used for storing the prediction
        ...
        try:
            ...
            log_file = open(self.log_path, 'a+')
            message = "Connection Unsuccessful with cassandra database due to Incorrect credentials"
            self.logger.log(log_file, message)
            log_file.close()
    ...
    def ee_create_table(self, column_names):
        ...
        :Description: This method created a 'good_pred_data' table to store good data
                      with the appropriate column names.
        :param column_names: Column Names as expected from schema based on DSA
        :return:None
    ...
    def ee_insert_good_pred_data(self):
        """
        :Method Name: ee_insert_good_pred_data
        :Description: This method uploads all the files in the good Data folder
                      to the good_pred_data table in cassandra database.
        :return: None
        :On Failure: Exception
        """
        try:
            ...
            for filename in os.listdir(self.good_file_dir):
                ...
                for i in range(len(temp_df)):
                    ...
                    if 'null' in temp_lis:
                        ...
                        tup = tuple(temp_lis)
                    insert_query = f"INSERT INTO good_pred_data({col_names}) VALUES {tup};"
                    print(insert_query)
                    session.execute(insert_query)

                log_file = open(self.log_path, "a+")
                message = f"Data in {filename} uploaded successfully to good_pred_data table"
                self.logger.log(log_file, message)
                log_file.close()

        except Exception as e:
            log_file = open(self.log_path, "a+")
            message = f"Error while uploading data to good_pred_data table: {str(e)}"
            self.logger.log(log_file, message)
            log_file.close()
            raise e
    ...
    def ee_data_from_db_to_csv(self):
        ...
        try:
            session = self.ee_db_connection()
            prediction_data_file = 'prediction_file.csv'
            col_name_query = "select column_name from system_schema.columns where " \
                             "keyspace_name='energy_efficiency_internship' and table_name='good_pred_data'; "
            headers = []
            ...
            get_all_data_query = "select * from good_pred_data;"
            ...
            with open(prediction_data_file, 'w', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow(headers)
                csv_writer.writerows(data)

            log_file = open(self.log_path, 'a+')
            message = f"All data from good data table saved to {prediction_data_file}"
            self.logger.log(log_file, message)
            log_file.close()
