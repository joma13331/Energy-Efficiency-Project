...
class EEDataFormatPred:
    """
    Class Name: EEDataFormatPred
    Description: This class shall be used for handling all the validation done on the Raw EEPrediction Data
    ...
    def __init__(self, path):
        ...
        """
        self.dir_path = path
        self.good_raw_path = "EEDIV/PredictionData/GoodRaw/"
        self.bad_raw_path = "EEDIV/PredictionData/BadRaw/"
        self.schema_path = "EESchema/prediction_schema.json"
        self.logger = EELogger()
        if not os.path.isdir("EElogging/prediction/"):
            os.mkdir("EElogging/prediction/")

        self.log_path = "EElogging/prediction/EEDataFormatPred.txt"

    def ee_values_from_schema(self):
        """
        :Method Name: ee_values_from_schema
        :Description: This method utilizes the training file EESchema from DSA to obtain
        ...
        try:
            ...
            # If in any of the above the values on the RHS is not Equal to no of
            # variables in LHS ValueError is Raised
            # If any of the above key is absent in schema_training.json then
            # KeyError is raised
    ...
    def ee_validating_file_name(self, regex):
        ...
        # delete the directories for good and bad data in case last run was
        # unsuccessful and folders were not deleted.
        self.ee_delete_existing_bad_data_folder()
    ...
    def ee_convert_direct_excel_to_csv(self):
        ...
        df.to_csv("prediction_file.csv", header=True, index=True, index_label="ID")
